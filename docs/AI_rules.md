...
- **Never trust client-submitted totals.** Per `API.md` §9.3, `subtotal`, `discountAmount`, `taxAmount`, and `grandTotal` are always recalculated server-side from `items` + `couponCode` + per-category `Category.vatRate` (VAT is informational and does not inflate `grandTotal`, per `DATABASE.md` §8.2). Any new money-calculating endpoint follows the same rule: server is the source of truth, client input is advisory.
...
- Soft-delete collections (`User`, `Customer`) use `isActive: false`. Vendor, Category, and Product use hard delete (not soft delete). Product retains `isActive` as an availability toggle for POS catalog filtering, not as a soft-delete flag.
- Hard-delete collections (`Coupon`, `Task`, `Salary`, `Income`, `Expense`) may be physically removed.
- `Order` is **never** hard-deleted by generated code. `Order` deletion follows the narrow restriction in `API.md` §10 (same-day, `pending`, no coupon usage) until the open item there is resolved — do not "simplify" this into a generic delete. **Sole exception:** the Settings data-management operations `POST /settings/reset` (Reset All Data) and `POST /settings/restore` (restore from backup) hard-delete `Order`, `Expense`, and `ActivityLog` by design (`API.md` §23, `DATABASE.md` §5.11). Any new destructive operation must be added to that list in the docs first, not invented inline.
...
- Date-range filters across Dashboard/Income/Reports/Orders/Expenses use the one shared `useDateRangeFilter` hook and the one shared `?range=&from=&to=` query shape. Don't build a module-specific date picker that emits a different param shape. **Exception:** Attendance's list keeps the `?from=&to=` shape documented in `API.md` §14 — it's a record-history filter (not a report range), and its strict query schema rejects the `range` discriminator (see `decision.md` "Attendance — Code-Review Reconciliation — 2026-08-13").
