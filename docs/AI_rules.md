...
- Hard-delete collections (`Coupon`, `Task`) may be physically removed.
- `Order` and `Expense` are **never** hard-deleted by generated code. `Order` deletion follows the narrow restriction in `API.md` §10 (same-day, `pending`, no coupon usage) until the open item there is resolved — do not "simplify" this into a generic delete. **Sole exception:** the Settings data-management operations `POST /settings/reset` (Reset All Data) and `POST /settings/restore` (restore from backup) hard-delete `Order`, `Expense`, and `ActivityLog` by design (`API.md` §23, `DATABASE.md` §5.11). Any new destructive operation must be added to that list in the docs first, not invented inline.
