...
| `grandTotal` | Number | ✓ | `subtotal - discountAmount`, **clamped at 0** — the combined discount (coupon + manual `discountPercent`) is capped at the subtotal, so `grandTotal` is never negative and cannot enter revenue sums as a negative value (VAT is informational — displayed on the bill as part of the total discount line, but does not affect the amount the customer pays) |
| `cashTendered` | Number | — | cash handed over at payment capture (required by the service layer when `payment.method === 'cash'`) |
| `changeAmount` | Number | — | `cashTendered - grandTotal`, **always recomputed server-side** at payment capture — a client-supplied `changeAmount` is never trusted (prevents "tendered 1000, change 0" cash skimming) |
...
1. **Order creation transaction** (per `ARCHITECTURE.md` §5): a single Mongo multi-document transaction covers (a) `Counter` increment for `orderNumber`, (b) the `Order` insert, (c) the `Coupon.usageCount` `$inc` when a coupon is applied (with rule 2's re-validation), (d) the `ActivityLog` write, and (e) the table-booking step (§5.7). Orders are always created with `paymentStatus: 'unpaid'`. If any step fails, the whole transaction rolls back — no partial order, no partial booking, no missing audit entry, no `usageCount` increment without a corresponding order.
2. **Coupon usage race condition:** two POS terminals applying the same near-limit coupon simultaneously is resolved by the atomic `$inc` happening *inside* the order-creation transaction (see rule 1), guarded by a conditional `findOneAndUpdate` (`usageLimit` null or `usageCount < usageLimit`) — not checked-then-acted-on as two separate operations.
