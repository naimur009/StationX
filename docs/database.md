...
1. **Order creation transaction** (per `ARCHITECTURE.md` §5): a single Mongo multi-document transaction covers (a) `Counter` increment for `orderNumber`, (b) the `Order` insert, (c) the `ActivityLog` write, and (d) the table-booking step (§5.7). Orders are always created with `paymentStatus: 'unpaid'` — `Coupon.usageCount` is **not** incremented here; the `$inc` is deferred to the payment-capture transaction (per `API.md` §9.3), which is where rule 2's re-validation applies. If any step fails, the whole transaction rolls back — no partial order, no partial booking, no missing audit entry.
2. **Coupon usage race condition:** two POS terminals applying the same near-limit coupon simultaneously is resolved by the atomic `$inc` happening *inside* the payment-capture transaction (see rule 1), combined with a service-layer check (`usageCount < usageLimit`) re-validated within the same transaction before commit — not checked-then-acted-on as two separate operations.
