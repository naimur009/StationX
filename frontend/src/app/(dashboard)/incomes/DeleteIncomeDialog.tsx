...
export default function DeleteIncomeDialog({ income, onClose }: DeleteIncomeDialogProps) {
    ...
    >
      <div className="space-y-5">
        ...
        <p className="text-sm text-slate-600">
          ...
          <span className="font-semibold text-slate-800">{formatCurrency(income.amount)}</span>?
        </p>

        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
          <div className="flex items-center justify-between">
            <span className="font-medium text-slate-800">{income.description}</span>
            <span className="text-slate-400">{income.category}</span>
          </div>
          <div className="mt-1 text-xs text-slate-400">
            {income.receivedFrom} &middot; {income.paymentMethod}
          </div>
        </div>
